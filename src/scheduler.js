...
const { collectPatents } = require('./collectors/kiprisApi');
const { collectPrice } = require('./collectors/priceTracker');
const { collectRegulations } = require('./collectors/regulationNews');
...
async function run() {
  ...
  try {
    // 뉴스 수집
    articles = await collectNews();
    logger.log(`수집 완료 — 뉴스 ${articles.length}건`);

    // 회사 목록 추출 (추가 데이터 수집 대상)
    const companies = extractCompanyList(articles);
    logger.log(`수집 완료 — 뉴스 ${articles.length}건, 회사 ${companies.length}개`);

    // 회사별 추가 데이터 수집
    for (const companyName of companies) {
      console.log(`[scheduler] "${companyName}" 추가 데이터 수집 중...`);
      ...
      try {
        patentResults[companyName] = await collectPatents(companyName);
      } catch (err) {
        logger.log(`특허 수집 실패 (${companyName}): ${err.message}`);
      }
      ...
      try {
        regulationResults[companyName] = await collectRegulations(companyName);
      } catch (err) {
        logger.log(`규제 뉴스 수집 실패 (${companyName}): ${err.message}`);
      }

      await sleep(2000);
    }
  } catch (err) {
    logger.log(`수집 단계 오류: ${err.message}`);
  }
