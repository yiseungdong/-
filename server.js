...
async function initDB() {
  ...
  try {
    ...
    try { await seedMoimGroups(); } catch (e) { console.error('⚠️ moim_groups 시드 실패 (무시):', e.message); }
...
initDB().then(() => {
  dbReady = true;
  console.log('✅ DB 준비 완료, 요청 수락 시작');
}).catch(err => {
  console.error('❌ DB 초기화 실패, 서버는 계속 실행:', err.message);
  dbReady = true; // 실패해도 서버는 실행 (기존 테이블로 동작)
});
...
// JSON 바디 파싱 — rate limiter보다 먼저 선언해야 req.body가 채워짐
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!dbReady) return res.status(503).json({ message: '서버 초기화 중입니다. 잠시 후 다시 시도해주세요.' });
